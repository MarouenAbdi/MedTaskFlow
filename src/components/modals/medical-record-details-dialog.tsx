...
import { FileText, Download, File as FilePdf, Mic, Copy, Eye, Edit, Save, X, CheckCircle2 } from "lucide-react";
...
export function MedicalRecordDetailsDialog({
  ...
  const handlePreviewFile = (attachment: typeof localRecord.attachments[0]) => {
    // Use the URL from the attachment if available
    if (attachment.url) {
      setPreviewFile({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  {isEditMode && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={() => handleEditField("symptoms", localRecord.symptoms)}
                    >
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  {isEditMode && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={() => handleEditField("diagnosis", localRecord.diagnosis)}
                    >
                      <Edit className="h-4 w-4 mr-2" />
                      Edit
                    </Button>
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  {isEditMode && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={() => handleEditField("treatment", localRecord.treatment)}
                    >
                      <Edit className="h-4 w-4 mr-2" />
                      Edit
                    </Button>
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  {isEditMode && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={() => handleEditField("notes", localRecord.notes)}
                    >
                      <Edit className="h-4 w-4 mr-2" />
                      Edit
                    </Button>
      ...
      <RichTextEditorModal
        ...
        title={`Edit ${editorFieldName?.charAt(0).toUpperCase() + editorFieldName?.slice(1) || ""}`}
        open={isEditorOpen}
